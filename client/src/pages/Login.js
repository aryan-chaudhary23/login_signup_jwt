...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if(state==="signup"){
      const{data}= await axios.post(backendUrl+'/api/auth/register',{name,email,password}) // data stores the response from the server
      if(data.success){
        setIsLoggedin(true)
        getUserData()
        navigate("/usage")
      ...
      const{data}= await axios.post(backendUrl+'/api/auth/login',{email,password}) // data stores the response from the server
      if(data.success){
        setIsLoggedin(true)
        getUserData()
        navigate("/usage")
